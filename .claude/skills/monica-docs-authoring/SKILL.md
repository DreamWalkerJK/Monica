...
Keep the skill entrypoint focused on task selection and execution. Put contracts, configuration, providers, and examples in its references, split only when readers benefit from loading them separately. Publish module usage through the owning `monica-infra-*` skill, using the catalog's `publication` metadata for coverage and source ownership. Use [docs-information-architecture.md](references/docs-information-architecture.md) for stable editorial guides and website projection; do not create another `docs/modules/` manual.
