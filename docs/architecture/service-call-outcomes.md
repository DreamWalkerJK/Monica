...
HTTP and remote presentation preserve declared application data and public metadata. They remove the reserved diagnostic keys `originResponse`, `request`, `response`, `deserializationError`, `exception`, `detail`, `chain`, and `chain_error`, including numbered variants. A deserialized typed error is projected back through the public model so undeclared technical members do not survive.
