...
8. **Success and payload**: `IsOk` accepts 200 and 201; it does not validate payload presence. Use `Res<T?>` when absence is part of the declared contract, and explicitly check data from remote calls before dereferencing it. `ResStatus` is the closed whitelist of supported transport outcomes mapping one-to-one to HTTP statuses; the synthetic 451/452/453/460 values are removed and semantics live in `ResultErrorCodes` reason codes.
