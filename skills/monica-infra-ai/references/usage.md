...
The host owns secret loading and model choice. `gpt-4o-mini` is included in Monica's current reserved model catalog; a custom model name must first be registered with `AddModel(...)` or the provider reports it as missing. `AddAIEndpoints().MapAIEndpoints()` is optional and maps stateless provider discovery under `/ai/providers` by default on a Web host; it is not the chat streaming API. Without `UseChatHistoryProvider<TProvider,TPartitionResolver>()`, chat works in memory and history writes report `NotPersisted`. Server-side history providers must isolate partitions by caller identity and secure their persisted agent state. The AI UI has a separate `UseBrowserChatHistory()` opt-in for persistence within one browser profile.
